...
- *IMapper* is the facade interface for mapping, the class behinds it is internal class *Mapper*, which wraps up sub classes of *RecursiveMapper* that contains the recursive mapping logic.
...
- *RecursiveMapper* and *RecursiveMappingContext* are internal classes, which expose some of their methods using corresponding public interfaces. Methods generated by *DynamicMethodBuilder* will need to call them to fulfill designed features.
- *MapperRegistry* prepares a lot of mapping related contents during mapping registration phase, then pass the integrated content to *Mapper* to implement features like keep unmatched items in collection navigation property, look up methods generated by *DynamicMethodBuilder* and check map to database type.
